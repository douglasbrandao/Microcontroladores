...
#include "BluetoothSerial.h" // inclusão da biblioteca do bluetooth
#include <string.h> // biblioteca de string c

#if !defined(CONFIG_BT_ENABLED) || !defined(CONFIG_BLUEDROID_ENABLED)
#error Bluetooth is not enabled! Please run `make menuconfig` to and enable it
#endif

BluetoothSerial SerialBT; // criação de um objeto BluetoothSerial nomeado como SerialBT
...
// definição dos pinos gpio do esp

const int red_pino = 15;
const int green_pino = 5;
const int blue_pino = 14;

// configuração frequencia pwm, canais e resolução de bits

const int frequencia = 1000; // frequencia de 1k hertz
const int red_canal = 0; // canal 0 pro vermelho
const int green_canal = 1; // canal 1 pro verde
const int blue_canal = 2; // canal 2 pro azul
const int bits = 8; // pwm de 8 bits 
...
void loop() {
  if (SerialBT.available()) { // verifica se está recebendo algum valor do bluetooth
    ...
    if(caractere == ')'){ // se for ')', ele entra nessa condição
       ...
       valores_hexa = ""; // zero a string objeto
       memset(valores_hexa_array, 0, sizeof(valores_hexa_array)); // limpo o vetor de caracteres do tipo primitivo
    }
  }
  delay(20);
}
